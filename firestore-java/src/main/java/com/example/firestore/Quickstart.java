...
/**
 * A simple Quick start application demonstrating how to connect to Firestore and add and query documents.
 */
public class Quickstart {

	private Firestore db;

	public Quickstart() throws Exception {
		// [START fs_initialize_project_id]
		// [START firestore_setup_client_create_with_project_id]
		Credentials credentials = GoogleCredentials.fromStream(Quickstart.class.getResourceAsStream("./google-services.json"));
	...
	void addDocument(String docName) throws Exception {
		switch (docName) {
			case "alovelace": {
				...
				data.put("born", 1815);data.put("last", "Lovelace");
			...
			case "cbabbage": {
				...
				Map<String, Object> data = new ImmutableMap.Builder<String, Object>().put("first", "Charles").put("last", "Babbage").put("born", 1791).build();
				ApiFuture<WriteResult> result = docRef.set(data);
				System.out.println("Update time : " + result.get().getUpdateTime());
				break;
			}
